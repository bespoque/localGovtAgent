      ...
      <div className="flex flex-wrap flex-row w-full mb-8">
        {items.map((item, key) => (
          <div
            ...
            <span
              className={`h-8 w-8 bg-teal-500 text-white flex items-center justify-center rounded-full text-lg font-display font-bold z-10`}>
              {key + 1}
            </span>
          </div>
        ))}
          ...
          <div
            ...
            className={`flex w-full lg:w-1/4 items-center p-2 justify-start ${
              item.disabled
                ...
                : 'cursor-pointer bg-teal-500 text-white'
            }`}
            onClick={() => console.log(item)}>
