...
const Todo = () => {
  const initialState = Array.from(Array(5).keys()).map((i) => {
    faker.locale = "en_US";
    let date = moment().subtract(i, "days").format("MMM DD");
    return {
      id: n++,
      ...
      date: date,
      img: `/images/faces/${faker.random.arrayElement(["m", "w"])}${
      ...
      <>
        {state.map((item, i) => (
          ...
          >
            ...
            <div className="ml-auto mr-1 lg:mr-2 whitespace-nowrap text-xs text-gray-500">
              {item.date}
            </div>
            <button
        ...
        {state.length > 0 && (
          <div className="flex flex-row items-center justify-end ml-auto space-x-2">
            {state.filter((item) => item.done).length > 0 && (
              <button
                className="btn btn-default bg-transparent hover:bg-teal-50 text-teal-500 hover:text-teal-600"
