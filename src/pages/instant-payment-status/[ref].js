...
const Index = () => {
  ...
  useEffect(() => {
    const refValuesArray = ref?.ref?.split(",");

    const firstValue = refValuesArray[0];
    const secondValue = refValuesArray[1];

    const fetData = async () => {
      setAuthToken();
      try {
        const response = await axios.get(`${url.BASE_URL}user/user-receipt/`, {
          params: { id: firstValue, state_id: secondValue },
        });
        setIsLoading(false);
  ...
  const paymentCallback = async (response) => {
    ...
    if (
      ...
      window.location = `/pending-payment/${paymentRef}`;
    }
  };
  ...
  const handleSubmitForm = async (event) => {
    ...
    const paymentRefInput = form.elements.payment_ref;

    const paymentRefValue = paymentRefInput.value;

    console.log("Payment Reference:", paymentRefValue);

    setUpdateRef(true);

    try {
      const response = await axios.post(
        `${url.BASE_URL}user/check-payment-status`,
      ...
      setUpdateRef(false); // Reset the loading state
      setOpenBank(false);
    }
  ...
  return (
    <>
      ...
      {data?.length > 0 &&
        data.map((da) => (
          <div key={da.idpymt}>
            <SectionTitle subtitle="nstant Payment" />
            <Widget>
                      ...
                      <div className="space-y-2 uppercase">
                        <div className="bg-gray-100  w-full p-2">
                          ...
                          <span className="text-black font-semibold">
                            {da.payer_details.taxpayer_name}
                          </span>
                        </div>
                        <div className="bg-gray-100  w-full p-2">
                          <h1 className="text-sm ">taxpayer type</h1>
                          <span className="text-black font-semibold">
                            {da.payer_details.taxpayer_type}
                          </span>
                        </div>
                        <div className="bg-gray-100  w-full p-2">
                          <h1 className="text-sm"> taxpayer id</h1>
                          <span className="text-black font-semibold">
                            {da.payer_details.taxpayer_id}
                          </span>
                        </div>
                        <div className="bg-gray-100  w-full p-2">
                          <h1 className="text-sm ">tax office</h1>
                          <span className="text-black font-semibold">
                            {da.payer_details.taxoffice}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="lg:w-4/5 w-full px-2">
                      ...
                      <div className="space-y-2 uppercase">
                        <div className="bg-gray-100  w-full p-2">
                          ...
                          <span className="text-black font-semibold">
                            {da.item_details.revenue_item}
                          </span>
                        </div>
                        <div className="bg-gray-100  w-full p-2">
                          <h1 className="text-sm"> Agency</h1>
                          <span className="text-black font-semibold">
                            {da.item_details.agency}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="uppercase lg:w-4/5 w-full px-2">
                      ...
                      <div className="shadow-lg w-full">
                        <div className="bg-gray-100  w-full p-2">
                          ...
                          <span className="text-black font-semibold">
                            {da.payment_details.payment_channel}
                          </span>
                        </div>
                        <div className="bg-gray-100  w-full p-2">
                          <h1 className="text-sm ">payment reference</h1>
                          <span className="text-black font-semibold">
                            {da.payment_details.payment_reference}
                          </span>
                        </div>
                        <div className="bg-gray-100  w-full p-2">
                          ...
                          <span className="text-black font-semibold">
                            {dateformat(
                              da.payment_details.creation_date,
                              "ddd, dS mmm, yyyy"
                            )}
                          </span>
                        </div>
                      ...
                      {da.payment_details.payment_channel == "Bank" ||
                        (da.payment_details.payment_channel == "USSD" && (
                          <button
                            className="py-2 mt-4 hover:text-green-800 text-black w-full text-left"
                            onClick={() => setOpenBank(true)}
                          >
                            Paid at bank or via USSD?
                            <span className="text-green-600"> Click</span> to
                    ...
                    <div className="bg-white text-gray-900 border-gray-200 dark:bg-gray-800 dark:text-white dark:border-gray-700 border-0 rounded-lg shadow-lg relative flex flex-col w-full outline-none">
                      ...
                      <form onSubmit={handleSubmit(PaymentHandler)}>
                        <div className="w-full px-8">
                          ...
                          <NewFormInput
                            label="Agency"
                            value={data[0].item_details.agency}
                            required
                            ref={register}
                            name=""
                          />
                          ...
                          <NewFormInput
                            label="Revenue Item"
                            value={data[0].item_details.revenue_item}
                            required
                            ref={register}
                            name="revItem"
                          />
                      ...
                      <form onSubmit={handleSubmitForm}>
                        <div className="w-full px-8">
                          ...
                          <NewFormInput
                            ...
                            value={da.payment_details.payment_reference}
                            required
                            ref={register}
                            name="payment_ref"
                          />
