    ...
    if (router && router.query) {
      ...
      const fetchPost = () => {
        ...
        if (paymentID.includes("FA")) {
          axios
            .get(
              `${urlNew}getpayment.php?paymentref=${paymentID}&by=assessment`
            )
          ...
          axios
            .get(`${urlNew}getpayment.php?paymentref=${paymentID}`)
            .then(function (response) {
              let res = response.data.body;
              setColData(() => response.data.body);
              console.log("res", res);
        ...
        <div className="flex justify-center">
          <div>
            <div className="flex justify-between my-3">
              <button
                className="btn  bg-green-600 btn-default text-white
            ...
            <div className="border p-6" ref={componentRef}>
              <p className="font-bold text-center">
                {colData?.pendingpayment ? "Incomplete Payment" : ""}
              </p>
              <p className="font-bold text-center">
                {colData?.amountpaid == "0" ? "Failed Payment" : ""}
              </p>
              <p>KOGI STATE GOVERNMENT</p>
              <section className="flex justify-between">
                <p className="font-bold">REVENUE RECEIPT</p>
                <p className="font-bold">{`Ref - ${
                  colData?.assessment_id || ""
                }`}</p>
              </section>
              ...
              <div className="flex justify-between">
                ...
                <div className="mt-6 mr-6">
                  <QRCode
                    value={`https://irs.kg.gov.ng/verify/verify_receipt.php?ref=${
                      colData?.assessment_id || ""
                    }`}
                    size={120}
                  />
              ...
              <div className="mt-3">
                ...
                <div className="grid grid-cols-6 gap-2">
                  ...
                  <div className="col-span-4">
                    <p className="font-bold">
                      NGN {formatNumber(colData?.amount || "")}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-6 gap-2">
                  ...
                  <div className="col-span-4">
                    <p className="font-bold">
                      NGN {formatNumber(colData?.amountpaid || "")}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-6 gap-2">
                  ...
                  <div className="col-span-4">
                    <p className="font-bold">
                      NGN{" "}
                      {Number(colData?.amount) - Number(colData?.amountpaid)}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-6 gap-2">
                ...
                <div className="grid grid-cols-6 gap-2">
                  ...
                  <p className="font-bold">
                    {" "}
                    {colData?.channel_id || colData?.pmt_meth}{" "}
                  </p>
                </div>
                <div className="grid grid-cols-6 gap-2">
                  <p>AGENCY:</p>
                ...
                <div className="border-b-2 mt-3 w-4/4 "></div>
              </div>
